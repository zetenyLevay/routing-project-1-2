...
public class GTFSCacheAStar {
    private static final Map<String, StopAStar> STOPS = new HashMap<>();
    ...
    private static void loadStops() {
        ...
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite::resource:gtfs.db");
            ...
            while (rs.next()) {
                ...
                StopAStar stop = new StopAStar(id, name, coord, locationType, parentStation);
                STOPS.put(id, stop);
            }
    ...
    public static Map<String, StopAStar> getStopsMap() {
        return Collections.unmodifiableMap(STOPS);
    }

    public static Collection<StopAStar> getAllStops() {
    ...
    public static StopAStar getStop(String stopId) {
